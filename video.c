...
float video(int w, int h, int durationMovie, int durationCredits, int fps, char* unit) {
   // YOUR CODE HERE - BEGIN
    ...
    float frame_size_color = (float)(w * h * 24); // 24 bits per pixel for color
    ...
    if (strcmp(unit, "bt") == 0) {
        ...
        return total_size_bits / 1000.0f; // Convert to kilobits
    } else if (strcmp(unit, "mo") == 0) {
        return total_size_bits / 1000000.0f; // Convert to megabits
    } else if (strcmp(unit, "go") == 0) {
        return total_size_bits / 1000000000.0f; // Convert to gigabits
    } else {
